...
server.tool("browser_navigate", "Navigate the embedded browser tab inside Zed/ACP to a URL. Use this instead of generic in-app browser navigation when the user asks for the Zed browser.", { url: z.string().describe("The URL to navigate to") }, async ({ url }) => {
    const result = await requireZedOk(await callZedAutomation("navigate", { url }));
    return textContent(`Navigated to ${result.url ?? url}`);
});
server.tool("browser_snapshot", "Capture an accessibility snapshot of the current page in the embedded Zed browser tab. Use this first for Zed browser tasks before screenshots, coordinate clicks, or external search.", {}, async () => {
    const result = (await requireZedOk(await callZedAutomation("snapshot")));
    const yaml = result.yaml ?? "";
    const header = result.ref_count != null
        ? `### Page snapshot (${result.ref_count} refs)\n\n`
        : "### Page snapshot\n\n";
    return textContent(`${header}${yaml}`);
});
server.tool("browser_click", "Click a snapshot element in the embedded Zed browser tab using a ref from browser_snapshot.", {
    element: z
        .string()
        .optional()
    ...
    await requireZedOk(await callZedAutomation("click", {
        ref: elementRef,
        target: elementRef,
...
server.tool("browser_type", "Type text into an editable element in the embedded Zed browser tab using a ref from browser_snapshot.", {
    element: z
        .string()
        .optional()
        .describe("Human-readable element description"),
    ...
    await requireZedOk(await callZedAutomation("type", {
        ref: elementRef,
        target: elementRef,
        text,
        submit: submit ?? false,
...
server.tool("browser_press_key", "Press a key on the keyboard in the embedded Zed browser tab", {
    key: z
        .string()
        .describe("Key name or character to press, e.g. Enter, ArrowLeft, Escape, a, " +
        "or a modifier chord like Control+a"),
}, async ({ key }) => {
    await requireZedOk(await callZedAutomation("press_key", { key }));
    return textContent(`Pressed ${key}`);
});
server.tool("browser_evaluate", "Evaluate JavaScript in the embedded Zed browser tab (optionally against an element)", {
    function: z
        .string()
    ...
    const result = (await requireZedOk(await callZedAutomation("evaluate", params)));
    return textContent(JSON.stringify(result.result ?? null, null, 2));
});
server.tool("browser_select_option", "Select option(s) in a <select> dropdown in the embedded Zed browser tab", {
    ...
    const result = (await requireZedOk(await callZedAutomation("select_option", { ref: elementRef, values })));
    return textContent(`Selected ${result.matched ?? 0} option(s); value=${result.value ?? ""}`);
});
server.tool("browser_hover", "Hover the mouse over an element in the embedded Zed browser tab", {
    element: z.string().optional().describe("Human-readable element description"),
...
server.tool("browser_file_upload", "Upload file(s) to a <input type=file> in the embedded Zed browser tab", {
    ...
    const result = (await requireZedOk(await callZedAutomation("file_upload", { ref: elementRef, paths })));
    return textContent(`Uploaded ${result.uploaded ?? 0} file(s) to ${elementRef}`);
});
server.tool("browser_drag", "Drag from one element to another in the embedded Zed browser tab (mouse-based)", {
    ...
    await requireZedOk(await callZedAutomation("drag", { startRef, endRef }));
    return textContent(`Dragged ${startRef} → ${endRef}`);
});
server.tool("browser_drop", "Drop data onto an element in the embedded Zed browser tab (synthetic HTML5 drop; data/MIME only, not files — use browser_file_upload for files)", {
    ...
    await requireZedOk(await callZedAutomation("drop", { ref: elementRef, data, mime }));
    return textContent(`Dropped onto ${elementRef}`);
});
server.tool("browser_handle_dialog", "Arm handling of JS dialogs (alert/confirm/prompt) in the embedded Zed browser tab. Call BEFORE the action that triggers the dialog; re-arm after navigation.", {
    ...
    const result = (await requireZedOk(await callZedAutomation("handle_dialog", { accept: accept ?? true, promptText })));
...
server.tool("browser_mouse_move_xy", "Move the mouse to (x, y) pixel coordinates in the embedded Zed browser tab", { x: z.number().describe("X in CSS px"), y: z.number().describe("Y in CSS px") }, async ({ x, y }) => {
    await requireZedOk(await callZedAutomation("mouse_move_xy", { x, y }));
    return textContent(`Moved to (${x}, ${y})`);
});
server.tool("browser_mouse_click_xy", "Click at (x, y) pixel coordinates in the embedded Zed browser tab", {
    ...
    await requireZedOk(await callZedAutomation("mouse_click_xy", { x, y, button: button ?? "left", doubleClick: doubleClick ?? false }));
    return textContent(`Clicked at (${x}, ${y})`);
});
server.tool("browser_mouse_down", "Press a mouse button at (x, y) in the embedded Zed browser tab (pair with browser_mouse_up)", {
    x: z.number().describe("X in CSS px"),
    y: z.number().describe("Y in CSS px"),
    button: z.enum(["left", "right", "middle"]).optional().describe("Mouse button (default left)"),
}, async ({ x, y, button }) => {
...
server.tool("browser_mouse_up", "Release a mouse button at (x, y) in the embedded Zed browser tab", {
    x: z.number().describe("X in CSS px"),
    y: z.number().describe("Y in CSS px"),
    button: z.enum(["left", "right", "middle"]).optional().describe("Mouse button (default left)"),
}, async ({ x, y, button }) => {
...
server.tool("browser_mouse_drag_xy", "Drag the mouse from (startX, startY) to (endX, endY) in the embedded Zed browser tab", {
    ...
    await requireZedOk(await callZedAutomation("mouse_drag_xy", { startX, startY, endX, endY, button: button ?? "left" }));
    return textContent(`Dragged (${startX}, ${startY}) → (${endX}, ${endY})`);
});
server.tool("browser_mouse_wheel", "Scroll the mouse wheel by (deltaX, deltaY) at (x, y) in the embedded Zed browser tab", {
    ...
    await requireZedOk(await callZedAutomation("mouse_wheel", { x: x ?? 0, y: y ?? 0, deltaX: deltaX ?? 0, deltaY: deltaY ?? 0 }));
    return textContent(`Wheel (${deltaX ?? 0}, ${deltaY ?? 0}) at (${x ?? 0}, ${y ?? 0})`);
});
server.tool("browser_console_messages", "Read buffered console messages (console.* + uncaught errors) from the embedded Zed browser tab", {
    level: z
    ...
    const result = (await requireZedOk(await callZedAutomation("console_messages", { level, clear: clear ?? false })));
...
server.tool("browser_network_requests", "List network requests (fetch/XHR) captured in the embedded Zed browser tab", {
    clear: z.boolean().optional().describe("Clear the network buffer after reading"),
}, async ({ clear }) => {
    const result = (await requireZedOk(await callZedAutomation("network_requests", { clear: clear ?? false })));
...
server.tool("browser_network_request", "Get one captured network request by id from the embedded Zed browser tab", {
    id: z.number().int().describe("Request id (from browser_network_requests)"),
}, async ({ id }) => {
    const result = (await requireZedOk(await callZedAutomation("network_request", { id })));
    return textContent(JSON.stringify(result.request ?? null, null, 2));
});
server.tool("browser_resize", "Resize the embedded Zed browser tab's viewport (for responsive testing)", {
...
server.tool("browser_pdf_save", "Save the current page as a PDF file from the embedded Zed browser tab", {
    ...
    const result = (await requireZedOk(await callZedAutomation("pdf_save", {
...
server.tool("browser_cookie_list", "List cookies for the current page in the embedded Zed browser tab", {}, async () => {
    const r = (await requireZedOk(await callZedAutomation("cookie_list")));
    const cs = r.cookies ?? [];
    const body = cs.length ? cs.map((c) => `${c.name}=${c.value}${c.domain ? ` (${c.domain})` : ""}`).join("\n") : "(no cookies)";
    return textContent(`### Cookies (${r.count ?? cs.length})\n${body}`);
});
server.tool("browser_cookie_get", "Get a cookie by name from the embedded Zed browser tab", { name: z.string().describe("Cookie name") }, async ({ name }) => {
    const r = (await requireZedOk(await callZedAutomation("cookie_get", { name })));
    return textContent(JSON.stringify(r.cookie ?? null, null, 2));
});
server.tool("browser_cookie_set", "Set a cookie in the embedded Zed browser tab (defaults to the current page's URL)", {
    name: z.string(),
    value: z.string(),
...
server.tool("browser_cookie_delete", "Delete a cookie by name from the embedded Zed browser tab", { name: z.string() }, async ({ name }) => {
    await requireZedOk(await callZedAutomation("cookie_delete", { name }));
    return textContent(`Deleted cookie ${name}`);
});
server.tool("browser_cookie_clear", "Clear all cookies in the embedded Zed browser", {}, async () => {
    await requireZedOk(await callZedAutomation("cookie_clear"));
    return textContent("Cleared all cookies");
});
function registerWebStorage(kind) {
    const store = kind;
    const pfx = kind === "local" ? "localstorage" : "sessionstorage";
    const label = kind === "local" ? "localStorage" : "sessionStorage";
    server.tool(`browser_${pfx}_get`, `Get a ${label} value by key in the embedded Zed browser tab`, { key: z.string() }, async ({ key }) => {
        const r = (await requireZedOk(await callZedAutomation("storage_get", { store, key })));
        return textContent(JSON.stringify(r.value ?? null));
    });
    server.tool(`browser_${pfx}_set`, `Set a ${label} key/value in the embedded Zed browser tab`, { key: z.string(), value: z.string() }, async ({ key, value }) => {
        await requireZedOk(await callZedAutomation("storage_set", { store, key, value }));
        return textContent(`Set ${label}[${key}]`);
    });
    server.tool(`browser_${pfx}_list`, `List all ${label} entries in the embedded Zed browser tab`, {}, async () => {
        const r = (await requireZedOk(await callZedAutomation("storage_list", { store })));
        const items = r.items ?? {};
        const keys = Object.keys(items);
        const body = keys.length ? keys.map((k) => `${k} = ${items[k]}`).join("\n") : `(${label} empty)`;
        return textContent(`### ${label} (${keys.length})\n${body}`);
    });
    server.tool(`browser_${pfx}_delete`, `Remove a ${label} key in the embedded Zed browser tab`, { key: z.string() }, async ({ key }) => {
        await requireZedOk(await callZedAutomation("storage_delete", { store, key }));
        return textContent(`Deleted ${label}[${key}]`);
    });
    server.tool(`browser_${pfx}_clear`, `Clear all ${label} in the embedded Zed browser tab`, {}, async () => {
...
server.tool("browser_storage_state", "Capture the current page's cookies + local/session storage to a JSON file (for auth/session reuse)", { filename: z.string().optional().describe("Output path; defaults to a temp file") }, async ({ filename }) => {
...
server.tool("browser_set_storage_state", "Restore cookies + storage into the current page from a saved storage-state file (or inline state)", {
    ...
    const r = (await requireZedOk(await callZedAutomation("set_storage_state", { state: payload })));
...
server.tool("browser_verify_element_visible", "Assert that a snapshot-ref element is visible in the embedded Zed browser tab (errors if not)", {
    ref: z.string().optional().describe("Snapshot ref to assert visible"),
    target: z.string().optional().describe("Alias for ref"),
}, async ({ ref, target }) => {
    const elementRef = ref ?? target;
    if (!elementRef)
        throw new Error("browser_verify_element_visible requires ref");
    await requireZedOk(await callZedAutomation("verify_element_visible", { ref: elementRef }));
    return textContent(`✓ ${elementRef} is visible`);
});
server.tool("browser_verify_list_visible", "Assert a snapshot-ref list is visible and has at least one item (errors if not)", {
    ref: z.string().optional().describe("Snapshot ref of the list"),
    target: z.string().optional().describe("Alias for ref"),
}, async ({ ref, target }) => {
    const elementRef = ref ?? target;
    if (!elementRef)
        throw new Error("browser_verify_list_visible requires ref");
    const r = (await requireZedOk(await callZedAutomation("verify_list_visible", { ref: elementRef })));
    return textContent(`✓ ${elementRef} is a visible list (${r.items ?? "?"} items)`);
});
server.tool("browser_verify_text_visible", "Assert text is visible on the page in the embedded Zed browser tab (errors if not)", { text: z.string().describe("Text expected to be visible") }, async ({ text }) => {
...
server.tool("browser_verify_value", "Assert a snapshot-ref element's value equals the expected value (errors if not)", {
    ...
    const elementRef = ref ?? target;
    if (!elementRef)
        throw new Error("browser_verify_value requires ref");
...
server.tool("browser_take_screenshot", "Take a screenshot (PNG/JPEG) of the embedded Zed browser tab", {
    fullPage: z
        .boolean()
        .optional()
    ...
    if (elementRef)
        params.ref = elementRef;
    const result = (await requireZedOk(await callZedAutomation("screenshot", params)));
    return {
        content: [
...
server.tool("browser_scroll", "Scroll the embedded Zed browser tab — by a pixel delta, or to bring a snapshot element into view", {
    ref: z
        .string()
        .optional()
    ...
    if (dy != null)
        params.dy = dy;
    const result = (await requireZedOk(await callZedAutomation("scroll", params)));
    return textContent(`Scrolled to x=${result.x ?? "?"}, y=${result.y ?? "?"} (maxY=${result.maxY ?? "?"})`);
});
server.tool("browser_tabs", "List, select, open, or close browser tabs in the embedded Zed browser", {
    action: z
    ...
    if (url)
        params.url = url;
    const result = (await requireZedOk(await callZedAutomation("tabs", params)));
...
server.tool("browser_fill_form", "Fill multiple form fields in one call in the embedded Zed browser tab", {
    fields: z
        .array(z.object({
    ...
    const result = (await requireZedOk(await callZedAutomation("fill_form", { fields })));
    return textContent(`Filled ${result.filled ?? 0} field(s)`);
});
server.tool("browser_close", "Close the active browser tab in the embedded Zed browser", {}, async () => {
...
server.tool("browser_record", "Record the automation run for codegen. action=start clears+enables the " +
    ...
    captureStorageState: z
        .boolean()
        .optional()
        .describe("On start: also capture cookies+storage so the generated spec can seed " +
        "auth via test.use({ storageState }) and skip the login flow"),
}, async ({ action, captureStorageState }) => {
    const result = (await requireZedOk(await callZedAutomation("record", { action, captureStorageState })));
    if (action === "start") {
        return textContent(`Recording started${result.storageStateCaptured ? " (storage state captured)" : ""}.`);
    }
    if (action === "stop") {
        return textContent(`Recording stopped — ${result.actions ?? 0} action(s) captured. Run browser_codegen to emit the spec.`);
    }
    return textContent(`Recording: ${result.recording ? "active" : "inactive"}, ` +
        `${result.actions ?? 0} action(s)` +
        `${result.storageStateCaptured ? ", storage state captured" : ""}.`);
});
server.tool("browser_codegen", "Generate a runnable Playwright .spec.ts from the current recording buffer " +
    ...
    const result = (await requireZedOk(await callZedAutomation("codegen")));
    ...
    return textContent(`Wrote Playwright spec to ${out}${note}\n\n` +
...
async function main() {
    ...
    try {
        ...
        console.error(`[zed-browser-mcp] Warning: Zed automation IPC not reachable (${err}). ` +
